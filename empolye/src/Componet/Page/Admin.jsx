...
const Admin = () => {
      ...
      .then(res => {
        if (res.data.Status) {
          setSummary({
            ...
            employeeCount: res.data.Result.totalEmployee,
            totalSalary: res.data.Result.totalSalary
          });
        }
      })
      .catch(err => console.log(err));
  ...
  const stats = [
    ...
    {
      label: "Total Salary",
      value: `฿${(summary.totalSalary || 0).toLocaleString()}`,
      icon: Wallet,
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/40 to-indigo-100/30">
      {/* 🌈 พื้นหลังกิมมิค: blob เคลื่อนไหว */}
