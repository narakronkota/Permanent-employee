...
const API_BASE_URL = "https://emolyee-contact.onrender.com";
const ADMIN_ID = 2; 
...
const Sidebar = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (res.data.Status) {
        ...
        Swal.fire('เกิดข้อผิดพลาด', res.data.Error || 'อัปโหลดไม่สำเร็จ', 'error');
      }
    } catch (err) {
  ...
  const removeAvatar = async () => {
    ...
    try {
      ...
      if (res.data.Status) {
        ...
        setShowPicker(false);
        Swal.fire('ลบเรียบร้อย!', 'รูปโปรไฟล์ถูกลบออกจากระบบแล้ว', 'success');
      } else {
        Swal.fire('ล้มเหลว', res.data.Error || 'ไม่สามารถลบรูปภาพได้', 'error');
      }
    } catch (err) {
  ...
  return (
    <aside className={`${collapsed ? "w-20" : "w-64"} min-h-screen bg-gradient-to-b from-slate-900 via-slate-900 to-indigo-500 text-white flex flex-col transition-all duration-300 shadow-2xl shadow-black/30 relative`}>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isUploading ? (
              ...
              <img src={adminAvatar} alt="Admin" className="w-full h-full object-cover bg-white" />
            ) : (
